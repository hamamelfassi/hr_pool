...
| Documents foldering | Defines family-level folder intent | May hold the vacancy folder pointer | Owns the runtime folder pointer and filed evidence | Medium | Implemented in source; verify template-derived fallback and runtime inheritance on install |
